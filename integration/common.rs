...
impl Ctx {
    pub async fn new() -> Self {
        ...
        let chunker = Arc::new(FixedSizeChunker::new(1024).unwrap());
    ...
    pub async fn create_project(&self) -> (String, String) {
        let resp = self
            ...
            .await;
        assert_eq!(resp.status(), StatusCode::CREATED, "create_project failed");
    ...
    pub async fn fork_branch(&self, project: &str, source: &str) -> (String, u64) {
        let resp = self
            ...
            .await;
        assert_eq!(resp.status(), StatusCode::CREATED, "fork_branch failed");
        let j = body_json(resp).await;
    ...
    pub async fn fork_at_lsn(&self, project: &str, source: &str, lsn: u64) -> String {
        let resp = self
            .raw(json_req(
                Method::POST,
                &format!("/projects/{project}/branches/{source}/fork"),
                json!({"lsn": lsn}),
            ))
            .await;
        ...
        let resp = self
            .raw(
                Request::builder()
                    .method(Method::GET)
                    .uri(format!(
                        "/projects/{project}/branches/{branch}/objects/{path}?at_lsn={lsn}"
                    ))
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    pub async fn delete_object(&self, project: &str, branch: &str, path: &str) -> StatusCode {
        self.raw(
            Request::builder()
                .method(Method::DELETE)
                .uri(format!(
    ...
    pub async fn restore(&self, project: &str, branch: &str, lsn: u64) -> (String, String) {
        let resp = self
            .raw(json_req(
                Method::POST,
                &format!("/projects/{project}/branches/{branch}/restore"),
                json!({"lsn": lsn}),
            ))
            .await;
        assert_eq!(
        ...
        let j = body_json(resp).await;
        let branch_id = j["branch"]["branch_id"].as_str().unwrap().to_string();
        let backup_id = j["backup_branch"]["branch_id"]
            .as_str()
            .unwrap()
            .to_string();
        (branch_id, backup_id)
    ...
    pub async fn merge(&self, project: &str, source: &str, target: &str) -> u64 {
        let resp = self
            .raw(json_req(
                Method::POST,
                &format!("/projects/{project}/branches/{source}/merge"),
                json!({"target_branch_id": target}),
            ))
            .await;
        assert_eq!(
            resp.status(),
        ...
        let j = body_json(resp).await;
        j["objects_merged"].as_u64().unwrap()
    }
}
